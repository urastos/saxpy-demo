...
class Config:
    ...
    def _streamGenConfig(self, core_id):
        streamN = int(self.coreConfigAppParams['array_length'])
        params = dict({
            "n"             : streamN // self.total_cores,
            "start_x"       : ( (streamN * 32) // self.total_cores ) * core_id,
            "start_y"       : ( (streamN * 32) // self.total_cores ) * core_id + (streamN * 32),
            "start_z"       : ( (streamN * 32) // self.total_cores ) * core_id + (2 * streamN * 32),
            "datasize"  : 32,
            "verbose"       : int(self.verbose)
            })
        return params
